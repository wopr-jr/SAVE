...
def parse_target_from_ckl_data(ckl_root) -> TARGET_ASSET:
    
    target_asset_data = ckl_root.find("ASSET")
    
    parsed_target_data = {
        "target_type": safe_extract_text(target_asset_data.find("ASSET_TYPE")),
        "host_name": safe_extract_text(target_asset_data.find("HOST_NAME")),
        "ip_address": safe_extract_text(target_asset_data.find("HOST_IP")),
        "mac_address": safe_extract_text(target_asset_data.find("HOST_MAC")),
        "fqdn": safe_extract_text(target_asset_data.find("HOST_FQDN")),
        "comments": safe_extract_text(target_asset_data.find("TARGET_COMMENT")),
        "role": safe_extract_text(target_asset_data.find("ROLE")),
        "is_web_database": safe_extract_text(target_asset_data.find("WEB_OR_DATABASE")),
        "technology_area": safe_extract_text(target_asset_data.find("TECH_AREA")),
        "web_db_site": safe_extract_text(target_asset_data.find("WEB_DB_SITE")),
        "web_db_instance": safe_extract_text(target_asset_data.find("WEB_DB_INSTANCE")),
        "classification": None,
    }

    asset_properties = ASSET_PROPERTIES(**parsed_target_data)

    target_asset = TARGET_ASSET(
        description: "Assest data extracted from a ckl file using SAVE program",
...
def parse_stigs_from_ckl_data(ckl_data) -> List[STIG]:
...
def extract_ckl_data(ckl_file) -> xml:
    ...
    if DEFUSEDXML_AVAILABLE:
        ckl_data = safe_parse(ckl_path)
    else:
        import xml.etree.ElementTree as ET
        ckl_data = ET.parse(ckl_path)

    return ckl_data

def parse_ckl(ckl_file) -> CKL_CHECKLIST:
    
    ckl_data = extract_ckl_data(ckl_file)
    ckl_root = ckl_data.get_root()
    target_asset = parse_target_from_ckl_data(ckl_root)
    stig_list = parse_stigs_from_ckl_data(ckl_root)
    
    ckl_properties = CKL_PROPERTIES(
        title="CKL checklist extracted from CKL file",
        target_data=target_asset,
        stigs=stig_list
    )

    ckl_checklist = CKL_CHECKLIST(properties=ckl_properties)
...
def _parse_stig_info(stig_info_el) -> dict:
    ...
    for si_data in stig_info_el.findall("SI_DATA"):
        name = safe_extract_text(si_data.find("SID_NAME"))
        data = safe_extract_text(si_data.find("SID_DATA"))
        if name:
            result[name] = data
    return result
...
def _parse_vuln(vuln_el) -> dict:
    ...
    for stig_data in vuln_el.findall("STIG_DATA"):
        attr_name = safe_extract_text

(stig_data.find("VULN_ATTRIBUTE"))
        attr_data = safe_extract_text
...
(vuln_el.find("STATUS"))
    ...
    return {
        ...
        "fixsafe_extract_text

": attrs.get("Fixsafe_extract_text
...
", ""),
        ...
        "status": status,
        "overrides": {},
        "comments": safe_extract_text

(vuln_el.find("COMMENTS")),
        "finding_details": safe_extract_text

(vuln_el.find("FINDING_DETAILS")),
...
def convert_ckl_to_cklb(ckl_file, cklb_path) -> str:
    ...
    target_data = {
        "target_type": safe_extract_text

(asset_el.find("ASSET_TYPE")) if asset_el is not None else "Computing",
        "host_name": safe_extract_text

(asset_el.find("HOST_NAME")) if asset_el is not None else "",
        "ip_address": safe_extract_text

(asset_el.find("HOST_IP")) if asset_el is not None else "",
        "mac_address": safe_extract_text

(asset_el.find("HOST_MAC")) if asset_el is not None else "",
        "fqdn": safe_extract_text

(asset_el.find("HOST_FQDN")) if asset_el is not None else "",
        "comments": safe_extract_text

(asset_el.find("TARGET_COMMENT")) if asset_el is not None else "",
        "role": safe_extract_text

(asset_el.find("ROLE")) if asset_el is not None else "None",
        "is_web_database": safe_extract_text

(asset_el.find("WEB_OR_DATABASE")).lower() == "true" if asset_el is not None else False,
        "technology_area": safe_extract_text

(asset_el.find("TECH_AREA")) if asset_el is not None else "",
        "web_db_site": safe_extract_text

(asset_el.find("WEB_DB_SITE")) if asset_el is not None else "",
        "web_db_instance": safe_extract_text

(asset_el.find("WEB_DB_INSTANCE")) if asset_el is not None else "",
        "classification": None,
    }
